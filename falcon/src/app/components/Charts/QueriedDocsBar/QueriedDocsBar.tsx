...
const CustomTick: React.FC<any> = ({ x, y, payload }) => {
  ...
  return (
    <g transform={`translate(${x},${y})`}>
      <Text x={0} y={0} dy={16} textAnchor="middle" fill="#344054" fontSize={12} fontWeight={400}>
        {truncatedLabel}
      </Text>
    </g>
  );
...
const QueriedDocsBar: React.FC<Props> = ({ data }) => {
  return (
                    ...
                    <div className="grid grid-cols-1 gap-1">
                      <div className="flex flex-col">
                        ...
                        <span className="font-medium text-xs text-grey-800">
                          {payload[0].value} times accessed
                        </span>
                      </div>
                    </div>
                  </div>
