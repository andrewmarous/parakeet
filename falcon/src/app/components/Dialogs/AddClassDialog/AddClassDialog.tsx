...
import { LuArrowUpDown, LuCode } from "react-icons/lu";
import AddCourseDropdownForm from "../../Forms/AddCourseDropdownForm/AddCourseDropdownForm";
import AddCourseForm from "../../Forms/AddCourseForm/AddCourseForm";
import { Button } from "../../Inputs/Button/Button";
import { Dialog, DialogContent, DialogTrigger } from "../Base/Dialog";
import isValidUser from "./actions";
...
export function AddClassDialogContent() {
  const [isValid, setIsValid] = useState(false);
  const contentRef = useRef<HTMLDivElement>(null);

  useEffect(() => {
    isValidUser().then((data) => setIsValid(data));
  }, []);

  return (
    <DialogContent className="w-[30rem]" ref={contentRef}>
      <div>
        <CourseIcon />
      </div>
            ...
            <div className="flex flex-row gap-3">
              <TabsList className="grid grid-cols-2 w-full gap-2">
                <TabsTrigger value="select">
                  {" "}
                  <LuArrowUpDown className="h-3 mr-2" /> Select course
                </TabsTrigger>
                <TabsTrigger value="courseCode">
                  <LuCode className="h-3 mr-2" /> Course code
                </TabsTrigger>
              </TabsList>
              <TooltipProvider>
                <Tooltip>
                  ...
                  <TooltipContent className="max-w-96">
                    <p className="text-grey-800">
                      If you are a Cornell student you can select your course.
                    </p>
                  </TooltipContent>
                </Tooltip>
              </TooltipProvider>
            </div>
            <TabsContent value="courseCode" className="w-full">
              <AddCourseForm />
            </TabsContent>
            <TabsContent value="select" className="w-full">
              <AddCourseDropdownForm containerRef={contentRef} />
            </TabsContent>
          </Tabs>
        ) : (
          <AddCourseForm />
